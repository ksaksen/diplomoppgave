...
In the $sp3s^*$ model we have 13 independent matrix elements to build up the energy vectors and the hopping matrices, equation 1.64:
...
V(s^*a, pc) & = 4 \langle s^*, a, \vec{R} | H | px, c, \vec{R} \rangle \\
V(pa, s^*c) & = 4 \langle px, a, \vec{R} | H | s^*, c, \vec{R} \rangle
\end{array} \quad (2.2)
$$

The 3 first lines represent 6 independent elements when we insert values for $b$; anion or cation. In this approximation $s^*s$ and $s^*s^*$ elements are set to zero.
As only nearest neighbors are interacting, the $sp^3s^*$ model then has a Hamilton operator as follows:
...
In article [1] $V(s^*, s^*)$ is included even though it's value is measured to be zero. I have also included the bondlengths to the set of features that is presented in equations 1.64, see at the end of the list. These bondlengths are also found from article [1].
...
Table 2.2: OVER: Empirical matrix elements of the $sp3s^*$ Hamiltonian in eV for AlSb.
UNDER: Empirical matrix elements of the $sp3s^*$ Hamiltonian in eV for InSb.
...
Table 2.3: Empirical matrix elements of the $sp3s^*$ Hamiltonian in eV for AlAs.
...
The energy vectors $E_a$ and $E_c$ in the $sp3s^*$ model have elements that deal with the energy of the orbitals at anion and cation layers. For the cations we get:
...
E_c(5) = E(s^*,c) = \langle s^*,c, \vec{R}|H|s^*,c, \vec{R} \rangle
$$
(2.4)
...
E_a(5) = E(s^*,a) = \langle s^*,a, \vec{R}|H|s^*,a, \vec{R} \rangle
$$
(2.5)
...
Uca(2,5) = -V(s^*a,pc) \cdot c2 = -4 \cdot c2 \langle s^*, a, \vec{R}|H|p, c, \vec{R} \rangle \\
...
Uca(5,2) = V(pa,s^*c) \cdot c2 = 4 \cdot c2 \langle px, a, \vec{R}|H|s^*, c, \vec{R} \rangle \\
Uca(5,5) = (0,0)
\tag{2.7}
$$
...
Uca(3,5) = -V(s^*a,pc) \cdot s2 = -4 \cdot s2 \langle s^*, a, \vec{R}|H|px, c, \vec{R} \rangle \\
...
Uca(5,1) = (0,0) \\
Uca(5,3) = V(pa,s^*c) \cdot c2 = 4 \cdot c2 \langle px, a, \vec{R}|H|s^*, c, \vec{R} \rangle \\
Uca(5,4) = -V(pa,s^*c) \cdot s2 = -4 \cdot s2 \langle px, a, \vec{R}|H|s^*, c, \vec{R} \rangle
\tag{2.8}
$$
...
Vca(2,5) &= V(s^*a,pc)*cl = 4*cl \langle s^*, a, \vec{R}|H|px, c, \vec{R} \rangle \\
...
Vca(5,1) &= (0,0) \\
Vca(5,2) &= -V(pa,s^*c)*cl = -4*cl \langle px, a, \vec{R}|H|s^*, c, \vec{R} \rangle \\
Vca(5,5) &= (0,0) \tag{2.9}
\end{aligned}
$$
...
Vca(3,5) &= V(s^*a,pc)*s1 = -4*s1 \langle s^*, a, \vec{R}|H|px, c, \vec{R} \rangle \\
Vca(4,1) &= -V(sa,pc)*s1 = -4*s1 \langle s, a, \vec{R}|H|px, c, \vec{R} \rangle \\
Vca(4,2) &= -V(x,y)*s1 = -4*s1 \langle px, a, \vec{R}|H|py, c, \vec{R} \rangle \\
Vca(4,5) &= V(s^*a,pc)*s1 = -4*s1 \langle s^*, a, \vec{R}|H|px, c, \vec{R} \rangle \\
Vca(5,3) &= V(pa,s^*c)*s1 = 4*s1 \langle px, a, \vec{R}|H|s^*, c, \vec{R} \rangle \\
Vca(5,4) &= V(pa,s^*c)*s1 = 4*s1 \langle px, a, \vec{R}|H|s^*, c, \vec{R} \rangle \tag{2.10}
\end{aligned}
$$
...
$$
\Gamma_{c,N+1}^{-} = \frac{1}{E-E_{c,N}-V_{ca}(\Gamma_{a,N}^{-})V_{ac}} \tag{2.12}
$$
$$
\Gamma_{a,N+1}^{-} = \frac{1}{E-E_{a,N}-U_{ac}(\Gamma_{c,N+1}^{-})U_{ca}} \tag{2.13}
$$
...
$$\Gamma_c^- = Acm \frac{1}{Aam} U_{a,c}^{-1} \tag{2.15}$$
...
$$
G_{00c} = \left( E-E_c-U_{c,a}(\Gamma_a^+)^{-1}U_{a,c}-V_{c,a}(\Gamma_a^-)^{-1}V_{a,c} \right)^{-1} \tag{2.16}
$$
...
$$
G_{00a} = \left( E-E_a-V_{a,c}(\Gamma_c^+)^{-1}V_{c,a}-U_{a,c}(\Gamma_c^-)^{-1}U_{c,a} \right)^{-1} \tag{2.17}
$$
